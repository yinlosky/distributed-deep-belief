"""This code has been modified from the original Theano training module
   for RBMs: http://deeplearning.net/tutorial/rbm.html
   to encorporate CRBMs in a Map function for Hadoop streaming

   We are using Theano in this project because the automated c compilation
   Theano provides is likely to be much faster than native Python or Java
   in handling data blocks, despite the initial overhead.
"""

#file IO imports
import os, sys, cPickle, zlib, StringIO, fileinput
...
class CRBM(object):
    ...
    def energy(self, v_term, h_term):
        ''' energy(self, v_term, h_term)
            Function to compute the energy '''
        hWv = T.dot(h_term, T.nnet.conv(v_term, numpy.fliplr(numpy.flipud(self.W))))
        hbias_term = T.dot(self.hbias, h_term)
        vbias_term = T.dot(self.vbias, v_term)
        return T.sum(T.log(1+T.exp(wx_b + hbias_term + vbias_term)),axis = 1)
...
class Map:
    """
        Hadoop streaming module, takes inputs from file handler and
        produces RBM output
    """

    # input args and their defaults
    argsdict = {'--n_chains' : ('n_chains',20),
                '--n_sample' : ('n_samples',10),
                '--learning' : ('learning_rate',0.1),
                '--n_hidden' : ('n_hidden',500),
                '--epochs' : ('training_epochs',15)}
    # store these in the pickled data
    # if coming from another source (e.g. java) the unpickler should handle these names
    key_names = ('weights','h_bias','v_bias','v_data','h_chain_states')

    def __init__(self, **kwargs):
        for var,default in argsdict.values():
            exec('self.%s = %s' % (var,kwargs.pop(var,default)))
        self.batch_size = len(kwargs.keys())
        self.data = kwargs
    ...
    def train_rbm(self):
        ...
        #incoming params
        for varname in self.key_names:
            exec('%s = []' % (varname,))
        
        for key in self.data:
            for varname in self.key_names:
                try:
                    exec('%s += [self.data[%s][%s]' % (varname,key,varname)]
                except KeyError:
                    exec('%s += []' % (varname,))
...
def zlibunpickle(dataobj):
    uncomp = zlib.decompress(dataobj)
    stringobj = StringIO.StringIO(uncomp)
    return cPickle.Unpickler(uncomp)

def gzunpickle(dataobj):
    import gzip
    tempname = os.tmpnam()
    with open(tempname,'wb') as tempfile:
        tempfile.write(dataobj)
        tempfile.close()
    with gzip.open(tempname,'rb') as f:
       return cPickle.Unpickler(f)
    
def parseInput(source,*args):
    sourcedict = {'gzpickle' : gzunpickle,
                  'zpickle' : zlibunpickle}

    argsdict = Map.argsdict
    
    output_vals = {}
    
    try:
        func = sourcedict[source]
    except:
        func = sourcedict['pickle']
    itr = 0
    while itr < len(args):
        thisarg = args[itr]
        try:
            name,default = argsdict[thisarg]
            hasNext = type(default) != bool
            if hasNext:
                itr += 1
                val = args[itr]
                output_vals[name] = val
            else:
                output_vals[name] = True
        except KeyError:
            logging.critical("The provided arguments were unknown, the mapper cannot proceed")
            sys.exit(1)
        except IndexError:
            logging.critical("Not enough arguments provided, the mapper cannot proceed")
            sys.exit(1)
        itr += 1
    for line in fileinput.input():
        hashval,data = line.split('\t')
        output_vals[hashval] = sourcedict[source](data)
    return output_vals

if __name__ == '__main__':
    kwargs = parseInput(sys.argv[-1], sys.argv[1:-1])
    mapper = Map(**kwargs)
    mapper.train_rbm()
    mapper.dump()
